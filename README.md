...
- **Numsolis** — пасьянс из 6 каскадных столбцов с двумя цветами, drag-and-drop стопок, Undo, тремя сложностями и целью собрать 2048 обоих цветов.
...
Numsolis использует одно общее поле из шести столбцов и ровно два цвета карт.
Любую карту можно перетащить вместе со всем хвостом над ней, включая целый
столбец. Карту можно положить на большее число любого цвета либо на равную карту
того же цвета для автоматического слияния. Если после хода столбец полностью
опустел, он закрывается: на его месте появляется заглушка с крестиком, и этот
столбец больше нельзя использовать как место назначения. В стартовой раздаче
есть только степени двойки от 2 до 1024; 2048 появляется только в результате
игры. Партия завершается, когда собрана 2048 каждого из двух цветов. В каждой
стопке максимум 9 карт, доступны Undo и уровни Easy / Medium / Hard. Генератор
строит расклады обратными ходами и затем перепроверяет известное решение с
учётом закрываемых столбцов.
...
Для Numsolis отдельно проверяются два цвета, отсутствие готовой 2048 в раздаче,
цепные merge-стадии, закрытие опустевших столбцов, запрет переносов в закрытый
столбец и воспроизведение гарантированного решения с этими ограничениями.
