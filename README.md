...
- **Numsolis** — пасьянс из 6 каскадных столбцов со степенями двойки, drag-and-drop стопок, Undo, тремя сложностями и целью собрать 2048.
...
Numsolis использует одно общее поле из шести столбцов. Любую карту можно
перетащить вместе со всем хвостом над ней, включая целый столбец. Карта кладётся
в пустой столбец, на большее число любого цвета либо на равную карту того же
цвета для автоматического слияния. В стартовой раздаче есть только степени
двойки от 2 до 1024; 2048 появляется только в результате игры и завершает
партию. В каждой стопке максимум 9 карт, доступны Undo и уровни Easy / Medium /
Hard. Генератор строит расклады обратными ходами от решения, поэтому каждая
новая партия имеет гарантированный путь к 2048.
...
- Numsolis: три сложности, гарантированный путь решения, перенос части/целого столбца, создание 2048 только из 1024 и Undo.
